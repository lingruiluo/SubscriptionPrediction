...
We encode the response variable `y` into 1 if the client subscribed to a term deposit and 0 otherwise.
...
```{r}
train <- read.csv("train.csv") %>% select(-X)
test <- read.csv("test.csv") %>% select(-X)
```
...
```{r, message = FALSE}
logit_pred = predict(logit_mod, test, type = "response")
logit_pred = ifelse(logit_pred > 0.5, 1, 0)
logit_pred_acc = mean(logit_pred == test$y)
cat("Prediction Accuracy Rate: ",logit_pred_acc)
```

The accuracy achieved is 0.9019132 To further invest on this model, we look at the confusion matrix.

```{r}
table(test$y, logit_pred > 0.5)
```

```{r message=FALSE}
logit_roc <- roc(test$y,logit_pred,direction="<")
...
The overall accuracy decreases to `r logit_pred_acc`, since it classifies more people who did not subscribe into subscribe. However, the false positive rate actually declines, the goal to target potential customers is better achieved without contacting too many clients.

Even though the overall accuracy is pretty much high, we cannot simply say it is good since we are starting from a pretty good starting point. We are going to try more advanced models next.
...
```{r message=FALSE, echo=FALSE}
nb_pred = as.numeric(predict(nb_model, test))-1
...
gb_model <- train(ifelse(y==0,'no','yes') ~ ., data = train, 
                 method = "gbm", 
...
```{r}
xgb_pred <- as.numeric(predict(xgb_model, test))-1
...
```{r message=FALSE}
xgb_roc <- roc(test$y,xgb_pred,direction="<")
xgb_auc <- auc(xgb_roc)
...
labels <- as.factor(train$y)
levels(labels) <- c('no','yes')
```
...
```{r}
cat_pred <- as.factor(predict(cat_model, test))
levels(cat_pred) <- c(0,1)
...
cat_roc <- roc(as.numeric(test$y),as.numeric(cat_pred),direction="<")
cat_auc <- auc(cat_roc)
