...
We also encode the response variable `y` into 1 if the client subscribed to a term deposit and 0 otherwise.
...
```{r, cache = TRUE}
logit_mod <- glm(y ~ ., data = data, family = binomial)
```
...
The overall accuracy decreases to 0.8692823, since it classifies more people who did not subscribe into subscribe. However, the false positive rate actually declines, the goal to target potential customers are better achieved without contacting too many clients.
